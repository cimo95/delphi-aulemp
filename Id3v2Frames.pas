...
const
  TFrameFlagValues : array[ TID3v2FrameVersions ] of array[ TFrameFlags ] of Byte =
    (
...
const
  ID3v2KnownFrames : array[ TFrameIDs ] of TID3v2FrameDescriptionData =
...
begin
  ...
  while i <= length( buf ) - 1 do begin
    ...
    if ( buf[ i ] = $FF ) and
      ( ( buf[ i + 1 ] = $00 ) or ( buf[ i + 1 ] >= $E0 ) ) then begin
      ...
      if buf[ length( buf ) - 1 ] = $FF then begin
        result := True ;
        Target.Write( zero , SizeOf( Zero ) ) ;
      end ;
      i := length( buf ) ;
...
begin
  ...
  while i <= length( buf ) - 1 do begin
    ...
    if ( buf[ i ] = $FF ) and ( buf[ i + 1 ] = $00 ) then begin
      Target.Write( buf[ last ] , i - last + 1 ) ;
      last := i + 2 ;
      inc( i , 2 ) ; // d.h. last = i
    end else begin
...
begin
  ...
  if FlagUnsynchronisation then begin
  ...
  DataOffset := 0 ;
  if fParsable then begin
    if FlagGroupingIndentity then begin
      inc( DataOffset ) ;
      SyncedStream.Read( fGroupID , SizeOf( fGroupID ) ) ;
    end ;
    if FlagDataLengthIndicator then begin
      inc( DataOffset , 4 ) ;
...
var
  tmpStream , UnsyncedStream : TMemoryStream ;
begin
  ...
  if fParsable then begin
    ...
    if UnsyncStream( tmpStream , UnsyncedStream ) then begin
      SetFlag( FF_Unsync )
    end else
      UnSetFlag( FF_Unsync ) ;
...
begin
  ...
  if ( ( fVersion = FV_2 ) and ( length( fIDString ) <> 3 ) )
    or
    ( ( fVersion <> FV_2 ) and ( length( fIDString ) <> 4 ) ) then begin
    result := False ;
    exit ;
  end ;

  for i := 1 to length( fIDString ) do
    if not ( fIDString[ i ] in [ '0'..'9' , 'A'..'Z' ] ) then begin
      result := False ;
      Break ;
    end ;
end ;
...
{$ENDIF}
    for i := low( TFrameIDs ) to High( TFrameIDs ) do
      if ID3v2KnownFrames[ i ].IDs[ fVersion ] = fIDString then begin
...
begin
  ...
  for i := 1 to length( aString ) do
    if Word( aString[ i ] ) > 255 then begin
      result := True ;
      break ;
    end ;
end ;
...
{$ENDIF}
    ...
    TE_UTF16 : begin
        ...
        if ( fData[ start ] = $FE ) and ( fData[ start + 1 ] = $FF ) then begin
...
begin
  if fParsable then begin
    ...
    i := 1 ;
    if ( enc = TE_UTF16 ) or ( enc = TE_UTF16BE ) then begin
...
begin
  ...
  inc( BytesWritten ) ;
  if UseUnicode then begin // Termination with two zeros (00 00)
    fData[ 1 + BytesWritten ] := 0 ;
    inc( BytesWritten ) ;
  end ;
    // set value
...
begin
  if fParsable then begin
    ...
    case fVersion of
      FV_2 : begin
          if length( fData ) <= 6 then {// 1 Enc, 3 Mime, 1 PicTyp, 1 Descr.-Terminierung -> 6 is minimum} begin
            Mime := '' ;
            PicType := 0 ;
            Description := '' ;
            result := False ;
        ...
        if length( fData ) <= 4 then {// 1 Enc, 1 mime-termination, 1 PicTyp, 1 Descr.-Terminierung -> this is minimum} begin
          Mime := '' ;
          PicType := 0 ;
          Description := '' ;
          result := False ;
