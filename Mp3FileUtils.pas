...
begin
  ...
  for i := 1 to length( ID ) do
    if not ( ID[ i ] in [ '0'..'9' , 'A'..'Z' ] ) then begin
      result := false ;
      Break ;
    end ;
end ;
...
begin
  ...
  if UseClusterSize and ( trim( Drive ) <> '' ) then begin
...
begin
  ...
  try
    Stream.Seek( -128 , soFromEnd ) ;
    Stream.Read( Buffer[ 1 ] , 3 ) ;
    if ( Buffer[ 1 ] = 'T' ) and ( Buffer[ 2 ] = 'A' ) and ( Buffer[ 3 ] = 'G' ) then begin
      Stream.Seek( -128 , soFromEnd ) ;
      SetStreamEnd( Stream ) ;
    end else
      result := ID3ERR_NoTag ;
...
var
  ...
  L : Integer ;
begin
  if AutoCorrectCodepage then begin
...
begin
  ...
  if fAutoCorrectCodepage then begin
...
begin
  result := -1 ;
  for i := 0 to Frames.Count - 1 do begin
    if ( TID3v2Frame( Frames[ i ] ).FrameType = FT_UserTextFrame ) then begin
      TID3v2Frame( Frames[ i ] ).GetUserText( iDescription ) ;
      if aDescription = iDescription then begin
        result := i ;
        break ;
      end ;
    end ;
  ...
  for i := 0 to Frames.Count - 1 do begin
    if ( Frames[ i ] as TID3v2Frame ).FrameIDString = IDstr then begin
      ...
      if ( ( Language = '*' ) or ( Language = '' ) ) or ( Language = iLanguage ) then
        Check := True ;
      if Check and ( ( Description = '*' ) or ( Description = iDescription ) ) then begin
        result := i ;
        break ;
      end ;
    end ;
...
begin
  ...
  for i := 0 to Frames.Count - 1 do begin
    if ( Frames[ i ] as TID3v2Frame ).FrameIDString = IDstr then begin
      ...
      if Description = iDescription then begin
        result := i ;
        break ;
      end ;
    end ;
...
begin
  ...
  for i := 0 to Frames.Count - 1 do
    if ( Frames[ i ] as TID3v2Frame ).FrameIDString = IDstr then begin
      ...
      if ( aEMail = iEMail ) or ( aEMail = '*' ) then begin
        result := i ;
        break ;
      end ;
    end ;
...
begin
  ...
  try
    ...
    if RawHeader.ID = 'ID3' then
      if RawHeader.Version in [ 2 , 3 , 4 ] then begin
...
begin
  ...
  try
    ...
    while ( Stream.Position<( FTagSize - fPaddingSize ) )
      and ( Stream.Position < Stream.Size ) do begin
      // read FrameID
      Stream.Read( FrameIDStr[ 1 ] , length( FrameIDStr ) ) ;

      if ValidFrame( FrameIDstr ) then begin
      ...
      {// (I ignore the ID3v2-footer)}begin
...
begin
  ...
  try
    ...
    try
      ...
      if ID3v2Stream.Size > 0 then begin
        ...
        if CacheAudio then begin
          ...
          try
            ...
            if TmpStream.CopyFrom( Stream , Stream.Size - Stream.Position ) <> AudioDataSize then begin
              TmpStream.Free ;
              result := ID3ERR_Cache ;
              Exit ;
            end ;
            ...
            Stream.Seek( -128 , soFromEnd ) ;
            v1Tag := '   ' ;
            if ( Stream.Read( v1Tag[ 1 ] , 3 ) = 3 ) then begin
              if ( v1Tag = 'TAG' ) then
                v1AdditionalPadding := 0
              else
...
begin
  ...
  try
    ...
    ExistingID3Tag.ReadHeader( Stream ) ;
      ...
      // if a Tag existiert...
    if ExistingID3Tag.FExists then begin
          // ...jump to its end...
      Stream.Seek( ExistingID3Tag.FTagSize , soFromBeginning ) ;
      ...
      try
        ...
        try
          ...
          if TmpStream.CopyFrom( Stream , Stream.Size - Stream.Position ) <> tmpsize then begin
            TmpStream.Free ;
            ExistingID3Tag.Free ;
            result := ID3ERR_Cache ;
            Exit ;
          ...
          Stream.Seek( 0 , soFromBeginning ) ;
          TmpStream.Seek( 0 , soFromBeginning ) ;
          if Stream.CopyFrom( TmpStream , TmpStream.Size ) <> TmpStream.Size then begin
            TmpStream.Free ;
            ExistingID3Tag.Free ;
            result := ID3ERR_Cache ;
            Exit ;
...
begin
  ...
  if ( language <> '*' ) and ( length( language ) <> 3 ) then language := 'eng' ;
...
begin
  ...
  if IDX <> -1 then begin
    if Stream = nil then
      ...
      if ( Description = '*' ) or ( MimeTyp = '*' ) or ( Stream.size = 0 ) then begin
    ...
    if ( Stream <> nil ) and ( Stream.Size > 0 ) then begin
      NewFrame := TID3v2Frame.Create( idStr , TID3v2FrameVersions( FVersion.Major ) ) ;
      NewFrame.AlwaysWriteUnicode := fAlwaysWriteUnicode ;
      newFrame.CharCode := fCharCode ;
      NewFrame.AutoCorrectCodepage := fAutoCorrectCodepage ;
...
begin
  ...
  if IDX <> -1 then begin
    ...
    if ( ( aRating = 0 ) and ( aCounter = 0 ) ) // set both to 0
      ...
      or ( ( aRating = -1 ) and ( currentRating = 0 ) and ( aCounter = 0 ) ) {// other (which is 0 atm) untouched} then
...
begin
  ...
  if IDX <> -1 then begin
    ...
    if assigned( Content ) and ( Content.Size > 0 ) then begin
      NewFrame := TID3v2Frame.Create( idStr , TID3v2FrameVersions( FVersion.Major ) ) ;
      NewFrame.AlwaysWriteUnicode := fAlwaysWriteUnicode ;
      newFrame.CharCode := fCharCode ;
      NewFrame.AutoCorrectCodepage := fAutoCorrectCodepage ;
...
begin
  ...
  if ( temp > 0 ) and ( temp < 10000 ) then begin
    Value := Trim( Value ) ;
...
begin
  ...
  while ( ( not erfolg ) and ( positionInStream + 3 < max ) ) do begin
    inc( bufferpos ) ;
    ...
    if ( bufferpos + 3 ) = ( length( buffer ) - 1 ) then begin
    ...
    if ( bufferpos + tmpMpegHeader.framelength + 3 > length( buffer ) - 1 ) // next header not in buffer
      and
      ( positionInStream + tmpMpegHeader.framelength + 3 < max ) {// but in stream} then begin
      ...
      try
        ...
        if not tmpXingheader.valid then begin
              // try VBRI
      ...
      if ( positionInStream + tmpMpegHeader.framelength + 3 > max ) then begin
        continue ;
      end ;
    ...
    // ---------------------------------------------------------------------------
    if not Skip3rdTest then begin
        // eventually: load more data
      if ( bufferpos + tmpMpegHeader.framelength + tmp2MpegHeader.framelength + 3 > length( buffer ) - 1 )
        and
        ( positionInStream + tmpMpegHeader.framelength + tmp2MpegHeader.framelength + 3 < max ) then begin
        ...
        if ( positionInStream + tmpMpegHeader.framelength + tmp2MpegHeader.framelength + 3 > max ) then continue ;

        if ( buffer[ bufferpos + tmpMpegHeader.framelength + tmp2MpegHeader.framelength ] <> $FF )
          or ( buffer[ bufferpos + tmpMpegHeader.framelength + tmp2MpegHeader.framelength + 1 ] < $E0 ) then continue ;
      end ;
    end ;
...
begin
  ...
  if ( abuffer[ position ] <> $FF ) or ( abuffer[ position + 1 ] < $E0 ) then begin
    result.valid := False ;
    exit ;
  end ;
  ...
  if ( Result.version = 0 ) or ( Result.Layer = 4 ) then begin
    Result.valid := False ;
    Exit ;
  end ;
  // <---
  ...
  if result.layer = 2 then
    if ( ( result.bitrate = 32 ) and ( result.channelmode <> 3 ) )
      ...
      or ( ( result.bitrate = 384 ) and ( result.channelmode = 3 ) ) then begin
      result.valid := false ;
      exit ;
    end ;
...
begin
  ...
  // --->
  // bugfix by terryk from delphi-forum.de
  if Length( abuffer )<=( position + xing_offset + 11 ) then begin
    Result.valid := False ;
    Exit ;
  end ;
  // <---
  ...
  if ( abuffer[ position + xing_offset ] = $58 ) {'Xing'}
    ...
    and ( abuffer[ position + xing_offset + 3 ] = $67 ) then {// Xing Tag found} begin
          // the next 4 bytes are flags, the 4th (= pos+7) is interesting here
    Xing_flags := abuffer[ position + xing_offset + 7 ] ;
    if ( Xing_flags and 1 ) = 1 then begin //number of frames in the next 4 bytes
...
begin
  ...
  if Length( abuffer )<=( position + vbriOffset + 16 ) then begin
  ...
  if ( abuffer[ position + vbriOffset ] = $56 ) // V
    ...
    and ( abuffer[ position + vbriOffset + 3 ] = $49 ) {// I} then {// VBRI found} begin
